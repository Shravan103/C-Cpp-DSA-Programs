From: John Hansen <John.Hansen@wizbang.co.nz>
Sent: Wednesday, December 24, 2025 7:43:39 AM (UTC+05:30) Chennai, Kolkata, Mumbai, New Delhi
To: Shawnon Guedes <shawnon@umangsoftware.com>
Cc: Jamie Hansen <Jamie.Hansen@wizbang.co.nz>; Amanda Milham <Amanda.Milham@wizbang.co.nz>
Subject: Sales by Staff Report Changes

Hey Shawnon

 

For some time, we’ve wanted to fix the Sales by Staff report (in otReports and Analytics), because the Staff breakdown is misleading.

 

It reports on who processed the invoice, rather than the staff members who took the orders that make up the invoice. This is a problem, especially with table service, because you most likely would have a situation where Jo, who is a waiter doing all the work, ordering and upselling on the floor, does not even appear in this report, because Fred is the cashier who creates the invoices, and so all the sales appear as Fred’s.

 

It would be fantastic to have, as an option (and make it the default), when reporting on sales by staff, a breakdown by who placed the orders rather than who created the invoice.

 

Mandy has created a script below and noted her thoughts.

 

Can you please check this out for me, and let me know your thoughts?
...
P: +64 9 360 8395 DDI: +64 9 984 0260 M: +64 21 022 14239 E: john.hansen@wizbang.co.nz

A: 2E/5 Ceres Court, Rosedale, Auckland 0632, New Zealand
...
From: Amanda Milham <Amanda.Milham@wizbang.co.nz>
Sent: Tuesday, 30 September 2025 3:10 pm
To: John Hansen <John.Hansen@wizbang.co.nz>; Bill Baduya <bill.baduya@wizbang.co.nz>
Cc: Jamie Hansen <Jamie.Hansen@wizbang.co.nz>
Subject: RE: Various Changes to Charts
...
Re 6., I went ahead and wrote some SQL to demonstrate how the invoice-order data should be joined, and this is, via the invoice_line.orderlineid to the order_line record (and then to the order record):

 

select

i.invoiceid, i.wheninvoiced, i.whoinvoiced, s1.staffid, s1.staffname, t1.terminalid, t1.terminalname,

--il.invoicelineid, il.itemid, il.qty, il.forb, il.itemabbrev, il.ilamount, il.discountamount, il.salestax, il.orderlineid,

o.orderid, o.whenordered, o.whoordered, s2.staffid, s2.staffname, t2.terminalid, t2.terminalname

 

from invoice i

inner join wblogin l1 on i.whoinvoiced = l1.loginid

inner join staff s1 on l1.staffid = s1.staffid

inner join terminal t1 on l1.terminalid = t1.terminalid
...
inner join wblogin l2 on o.whoordered = l2.loginid

inner join staff s2 on l2.staffid = s2.staffid

inner join terminal t2 on l2.terminalid = t2.terminalid

 

where i.wheninvoiced between '20.07.2021, 17:49:25.993' and '20.07.2021, 18:08:17.718'

 

 

Results below show that some orders (on the right) are ordered by a staff member and/or at a terminal that differs from the staff member who invoiced it and/or what terminal they were invoiced at, e.g., ordered on Mobile1 but invoiced/paid for at Cashier, by a different staff member:
...
type
  (**
    Report that shows each staff members sales.
  *)
  TformQRStaffSales = class(TForm)
    QRStaffSales: TQuickRep;
    bndTitle: TQRBand;
    lblHGroup: TQRLabel;
    lblHItem: TQRLabel;
    lblHUnits: TQRLabel;
    ...
    shpHeader: TQRShape;
    lblTime: TQRLabel;
    lblHWizBang: TQRLabel;
    lblHWizBangTM: TQRLabel;
    lblHPage: TQRLabel;
    txtHPageNo: TQRSysData;
    lblHGross: TQRLabel;
    lblHDiscount: TQRLabel;
    lblHForB: TQRLabel;
    lblTitle1: TQRLabel;
    lblTitle2: TQRLabel;
    lblTitle3: TQRLabel;
    ...
    txtPrice: TQRDBText;
    txtGross: TQRDBText;
    txtDiscount: TQRDBText;
    txtSalesTax: TQRDBText;
    txtItemGroup: TQRDBText;
    txtItem: TQRDBText;
    QRBSummary: TQRBand;
    lblTotals: TQRLabel;
    shpTotalsUnits: TQRShape;
    shpTotalsGross: TQRShape;
    shpTotalsNett: TQRShape;
    shpTotalsPrice: TQRShape;
    shpTotalsDiscount: TQRShape;
    txtTotalsUnits: TQRExpr;
    txtTotalsPrice: TQRExpr;
    txtTotalsGross: TQRExpr;
    txtTotalsDiscount: TQRExpr;
    txtTotalsNett: TQRExpr;
    shpTotalsBGross: TQRShape;
    shpTotalsBPrice: TQRShape;
    shpTotalsBUnits: TQRShape;
    shpTotalsBDiscount: TQRShape;
    shpTotalsBNett: TQRShape;
    txtOutletHeader: TQRExpr;
    txtStaffHeader: TQRExpr;
    QRBOutletFooter: TQRBand;
    lblOutletUnits: TQRLabel;
    lblOutletPrice: TQRLabel;
    lblOutletGross: TQRLabel;
    lblOutletDiscount: TQRLabel;
    lblOutletNett: TQRLabel;
    shpOutletNett: TQRShape;
    shpOutletDiscount: TQRShape;
    shpOutletGross: TQRShape;
    shpOutletPrice: TQRShape;
    shpOutletUnits: TQRShape;
    shpOutletBUnits: TQRShape;
    shpOutletBPrice: TQRShape;
    shpOutletBGross: TQRShape;
    shpOutletBDiscount: TQRShape;
    shpOutletBNett: TQRShape;
    txtOutletFooter: TQRExpr;
    QRBStaffFooter: TQRBand;
    lblStaffUnits: TQRLabel;
    lblStaffPrice: TQRLabel;
    lblStaffGross: TQRLabel;
    lblStaffDiscount: TQRLabel;
    lblStaffNett: TQRLabel;
    shpStaffNett: TQRShape;
    shpStaffDiscount: TQRShape;
    shpStaffGross: TQRShape;
    shpStaffPrice: TQRShape;
    shpStaffUnits: TQRShape;
    txtStaffFooter: TQRExpr;
    ChildBand: TQRChildBand;
    QRBSuperItemGroupFooter: TQRBand;
    txtSIGFooter: TQRExpr;
    lblSIGUnits: TQRLabel;
    shpSIGUnits: TQRShape;
    lblSIGPrice: TQRLabel;
    lblSIGGross: TQRLabel;
    lblSIGDiscount: TQRLabel;
    lblSIGNett: TQRLabel;
    shpSIGNett: TQRShape;
    shpSIGDiscount: TQRShape;
    shpSIGGross: TQRShape;
    shpSIGPrice: TQRShape;
    QRBForBFooter: TQRBand;
    txtForBFooter: TQRExpr;
    lblForBUnits: TQRLabel;
    shpForBUnits: TQRShape;
    lblForBPrice: TQRLabel;
    lblForBGross: TQRLabel;
    lblForBDiscount: TQRLabel;
    lblForBNett: TQRLabel;
    shpForBNett: TQRShape;
    shpForBDiscount: TQRShape;
    shpForBGross: TQRShape;
    shpForBPrice: TQRShape;
    QRBGroupFooter: TQRBand;
    txtGroupFooter: TQRExpr;
    lblGroupUnits: TQRLabel;
    lblGroupPrice: TQRLabel;
    lblGroupGross: TQRLabel;
    lblGroupDiscount: TQRLabel;
    lblGroupNett: TQRLabel;
    shpGroupNett: TQRShape;
    shpGroupDiscount: TQRShape;
    shpGroupGross: TQRShape;
    shpGroupPrice: TQRShape;
    shpGroupUnits: TQRShape;
    txtSuperItemGroupHeader: TQRExpr;
    lblHSalesTax: TQRLabel;
    lblNett: TQRLabel;
    lblGroupSalesTax: TQRLabel;
    shpGroupSalesTax: TQRShape;
    lblForBSalesTax: TQRLabel;
    shpForBSalesTax: TQRShape;
    lblSIGSalesTax: TQRLabel;
    shpSIGSalesTax: TQRShape;
    lblStaffSalesTax: TQRLabel;
    shpStaffSalesTax: TQRShape;
    lblOutletSalesTax: TQRLabel;
    shpOutletSalesTax: TQRShape;
    shpOutletBSalesTax: TQRShape;
    shpTotalsSalesTax: TQRShape;
    txtTotalsSalesTax: TQRExpr;
    shpTotalsBSalesTax: TQRShape;
    QRELocationHeaderName: TQRExpr;
    QRBLocationFooter: TQRBand;
    QRExprLocationFooter: TQRExpr;
    lblLocationUnits: TQRLabel;
    lblLocationPrice: TQRLabel;
    lblLocationGross: TQRLabel;
    lblLocationDiscount: TQRLabel;
    lblLocationNett: TQRLabel;
    shpLocationNett: TQRShape;
    shpLocationDiscount: TQRShape;
    shpLocationGross: TQRShape;
    shpLocationPrice: TQRShape;
    shpLocationUnits: TQRShape;
    lblLocationSalesTax: TQRLabel;
    shpLocationSalesTax: TQRShape;
    QRGGroup: TQRGroup;
    QRGStaff: TQRGroup;
    QRGForb: TQRGroup;
    QRGSuperItemGroup: TQRGroup;
    QRGOutlet: TQRGroup;
    QRGLocation: TQRGroup;
//    IvTranslator1: TIvTranslator;
    procedure FormCreate(Sender: TObject);
    procedure FormDestroy(Sender: TObject);
    procedure QRStaffSalesBeforePrint(Sender: TCustomQuickRep; var PrintReport: Boolean);
    ...
    procedure QRGLocationBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
    procedure QRGStaffBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
    ...
    procedure QRBGroupFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
    procedure QRBForBFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
    procedure QRBStaffFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
    procedure QRBLocationFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
    procedure QRBOutletFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
    ...
    procedure QRBSuperItemGroupFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
  private
    NettExcludeSalesTax: Boolean;

    TheTime : TDateTime;
    TotalsOnly : Boolean;     //Jon 13-10-2003

    STOutletUnits: Currency;
    STOutletGross: Currency;
    STOutletDiscount: Currency;
    STOutletSalesTax: Currency;
    STOutletNett: Currency;

    STLocationUnits: Currency;
    STLocationGross: Currency;
    STLocationDiscount: Currency;
    STLocationSalesTax: Currency;
    STLocationNett: Currency;

    STStaffUnits: Currency;
    STStaffGross: Currency;
    STStaffDiscount: Currency;
    STStaffSalesTax: Currency;
    STStaffNett: Currency;

    STSIGUnits: Currency;
    STSIGGross: Currency;
    STSIGDiscount: Currency;
    STSIGSalesTax: Currency;
    STSIGNett: Currency;

    STForBUnits: Currency;
    STForBGross: Currency;
    STForBDiscount: Currency;
    STForBSalesTax: Currency;
    STForBNett: Currency;

    STGroupUnits: Currency;
    STGroupGross: Currency;
    STGroupDiscount: Currency;
    STGroupSalesTax: Currency;
    STGroupNett: Currency;
    STGroupCount: Integer;
    ...
    FMultipleStaff: Boolean;

    procedure ShowGroupFooter(B: Boolean);
  public
    WBCSV: TWBCSV;
    property MultipleStaff: Boolean read FMultipleStaff write FMultipleStaff;
  end;
{******************************************************************************}
var
  formQRStaffSales: TformQRStaffSales;
{******************************************************************************}
implementation
{$R *.DFM}
{*****************************************************************************}
uses
  UDM, UReports, UAppDetails, uResources, DB {sinu};
{*****************************************************************************}
procedure TformQRStaffSales.FormCreate(Sender: TObject);
begin
  WBCSV := TWBCSV.Create;
  FMultipleStaff := False;
end;
{*****************************************************************************}
procedure TformQRStaffSales.FormDestroy(Sender: TObject);
begin
  WBCSV.Free;
end;
{*****************************************************************************}
procedure TformQRStaffSales.QRStaffSalesBeforePrint(Sender: TCustomQuickRep; var PrintReport: Boolean);
begin
  NettExcludeSalesTax:= formReports.chkSalesNettExcludeSalesTax.Checked;
  ...
  CurrentHeight:= Trunc(CurrentWidth * (QRStaffSales.Page.Length / QRStaffSales.Page.Width));
  ...
  with formReports do begin
    with (WBCSV) do begin
      if (Active) then begin
        ...
        if (radDGTDetails.Checked) then begin
          SendString(sFB);
          SendString(sGrp);
          SendString(sItem);
        end
        else if ((radDGTGroups.Checked) and (radAllItems.Checked)) then begin
          SendString(sGrp);
        end
        else begin
          SendEmpty(1);
        ...
        SendString(sPrice);
        SendString(sGross);
        SendString(sRepDiscount);
        SendString(sSalesTax);
        SendString(sNett);
        WriteLine;
      end;
    end;

    TotalsOnly:= radDGTTotals.Checked;

//--TitleBand
    lblHForB.Enabled:= radDGTDetails.Checked;
    lblHItem.Enabled:= radDGTDetails.Checked;
    if (radDGTDetails.Checked) then begin
      lblHGroup.Left:= ((24 * CurrentWidth) div CreatedWidth);
      lblHGroup.Enabled:= True;
    end
    else if (radDGTGroups.Checked and radAllItems.Checked) then begin
      lblHGroup.Left:= 0;
      lblHGroup.Enabled:= True;
      ...
      lblHGroup.Enabled:= False;
    end;

//--Group Subtotal Bands
    if (((ckbGroupGroup.Checked) and (radDGTDetails.Checked) and (radGroup.Checked)) or
          (radDGTGroups.Checked and radAllItems.Checked)) then begin
      QRGGroup.Enabled:= True;
      QRBGroupFooter.Enabled:= True;
    end
    else begin
      QRGGroup.Enabled:= False;
      QRBGroupFooter.Enabled:= False;
    end;
    if (radDGTGroups.Checked) then begin
      ShowGroupFooter(True);

      txtGroupFooter.Expression:= 'qrStaffSales.ITEMGROUPABBREV';

      shpGroupUnits.Enabled:= False;
      shpGroupPrice.Enabled:= False;
      shpGroupGross.Enabled:= False;
      shpGroupDiscount.Enabled:= False;
      shpGroupSalesTax.Enabled:= False;
      shpGroupNett.Enabled:= False;

      txtGroupFooter.Left:= 0;

      QRBGroupFooter.Height:= ((19 * CurrentHeight) div CreatedHeight);
    end
    else if ((ckbGroupGroup.Checked) and (radDGTDetails.Checked)) then begin
      ShowGroupFooter(True);

      txtGroupFooter.Expression:= sSubTotalExpr + ' + qrStaffSales.ITEMGROUPABBREV';

      shpGroupUnits.Enabled:= True;
      shpGroupPrice.Enabled:= True;
      shpGroupGross.Enabled:= True;
      shpGroupDiscount.Enabled:= True;
      shpGroupSalesTax.Enabled:= True;
      shpGroupNett.Enabled:= True;

      txtGroupFooter.Left:= ((8 * CurrentWidth) div CreatedWidth);

      QRBGroupFooter.Height:= ((25 * CurrentHeight) div CreatedHeight);
    end;

//--Staff Subtotal Bands
    QRGStaff.Enabled := (((ckbGroupStaff.Checked) and (radStaffAll.Checked or FMultipleStaff)) or (not radDGTDetails.Checked));
    QRBStaffFooter.Enabled := QRGStaff.Enabled;

    if (cmbOutlets.ItemIndex = 0) then begin
      txtStaffFooter.Expression:= 'qrStaffSales.STAFFNAME+'' at ''+qrStaffSales.OUTLETNAME';
    end
    else begin
      txtStaffFooter.Expression:= 'qrStaffSales.STAFFNAME';
    end;

    if (radDGTTotals.Checked or
          (radDGTGroups.Checked and radItemGroup.Checked)) then begin
      if ((ckbGroupForB.Checked) and (radFAndB.Checked)
          and (radDGTTotals.Checked) and (radGroup.Checked)
          and (radAllItems.Checked)) then begin
        //Partially Limit the data shown
        //This is when we are showing the ForB Totals
        txtStaffHeader.Enabled:= False;
        QRGStaff.Height:= ((8 * CurrentHeight) div CreatedHeight);

        txtStaffFooter.Expression:= 'qrStaffSales.STAFFNAME';
        txtStaffFooter.Font.Style:= [fsBold];
        shpStaffUnits.Enabled:= True;
        shpStaffPrice.Enabled:= True;
        shpStaffGross.Enabled:= True;
        shpStaffDiscount.Enabled:= True;
        shpStaffSalesTax.Enabled:= True;
        shpStaffNett.Enabled:= True;
      end
      else begin
        //Limit the data shown
        txtStaffHeader.Enabled:= False;
        QRGStaff.Height:= ((0 * CurrentHeight) div CreatedHeight);

        txtStaffFooter.Expression:= 'qrStaffSales.STAFFNAME';
        txtStaffFooter.Font.Style:= [];
        shpStaffUnits.Enabled:= False;
        shpStaffPrice.Enabled:= False;
        shpStaffGross.Enabled:= False;
        shpStaffDiscount.Enabled:= False;
        shpStaffSalesTax.Enabled:= False;
        shpStaffNett.Enabled:= False;
      end;
    end
    else begin
      txtStaffHeader.Enabled:= True;
      QRGStaff.Height:= ((33 * CurrentHeight) div CreatedHeight);

      txtStaffFooter.Expression:= sSubTotalExpr + ' +qrStaffSales.STAFFNAME';
      txtStaffFooter.Font.Style:= [fsBold];
      shpStaffUnits.Enabled:= True;
      shpStaffPrice.Enabled:= True;
      shpStaffGross.Enabled:= True;
...
//--ForB Subtotal Bands
    if ((ckbGroupForB.Checked) and (radFAndB.Checked)
        and ((not radDGTDetails.Checked) or radGroup.Checked)
        and (radAllItems.Checked)) then begin
      QRGForb.Enabled:= True;
      QRBForbFooter.Enabled:= True;
      if (radDGTTotals.Checked) then begin
        txtForBFooter.Expression:= 'IF(qrStaffSales.FORB = ''F'', ' + sFoodExpr + ' , ' + sBevExpr + ' )';

        shpForBUnits.Enabled:= False;
        shpForBPrice.Enabled:= False;
        shpForBGross.Enabled:= False;
        shpForBDiscount.Enabled:= False;
        shpForBSalesTax.Enabled:= False;
        shpForBNett.Enabled:= False;

        QRBForBFooter.Height:= ((19 * CurrentHeight) div CreatedHeight);
      end
      else begin
        txtForBFooter.Expression:= 'IF(qrStaffSales.FORB = ''F'', ' + sFoodExpr + ' , ' + sBevExpr + ' ) + ' + sQuotedSubTotals;

        shpForBUnits.Enabled:= True;
        shpForBPrice.Enabled:= True;
        shpForBGross.Enabled:= True;
        shpForBDiscount.Enabled:= True;
        shpForBSalesTax.Enabled:= True;
        shpForBNett.Enabled:= True;

        QRBForBFooter.Height:= ((23 * CurrentHeight) div CreatedHeight);
      end;
    end
    else begin
      QRGForb.Enabled:= False;
      QRBForbFooter.Enabled:= False;
    end;

//--Super Item Group Subtotal Bands
    if ((ckbGroupForB.Checked) and (radFAndB.Checked)
        and ((not radDGTDetails.Checked) or radGroup.Checked)
        and (radAllItems.Checked)) then begin
      QRGSuperItemGroup.Enabled:= True;
      QRBSuperItemGroupFooter.Enabled:= True;
      if (radDGTTotals.Checked) then begin
        txtSIGFooter.Expression:= 'qrStaffSales.SUPERITEMGROUP';

        shpSIGUnits.Enabled:= False;
        shpSIGPrice.Enabled:= False;
        shpSIGGross.Enabled:= False;
        shpSIGDiscount.Enabled:= False;
        shpSIGSalesTax.Enabled:= False;
        shpSIGNett.Enabled:= False;

        QRBSuperItemGroupFooter.Height:= ((19 * CurrentHeight) div CreatedHeight);
      end
      else begin
        txtSIGFooter.Expression:= 'qrStaffSales.SUPERITEMGROUP + ' + sQuotedSubTotals;

        shpSIGUnits.Enabled:= True;
        shpSIGPrice.Enabled:= True;
        shpSIGGross.Enabled:= True;
        shpSIGDiscount.Enabled:= True;
        shpSIGSalesTax.Enabled:= True;
        shpSIGNett.Enabled:= True;

        QRBSuperItemGroupFooter.Height:= ((23 * CurrentHeight) div CreatedHeight);
      end;
    end
    else begin
      QRGSuperItemGroup.Enabled:= False;
      QRBSuperItemGroupFooter.Enabled:= False;
    end;

//--Outlet Subtotal Bands
    QRGOutlet.Enabled:= ((ckbGroupOutlet.Checked) and (cmbOutlets.ItemIndex = 0) and (radGroup.Checked) and (chkOutletBreakdown.Checked));
    QRBOutletFooter.Enabled:= ((ckbGroupOutlet.Checked) and (cmbOutlets.ItemIndex = 0) and (radGroup.Checked) and (chkOutletBreakdown.Checked));
    QRGOutlet.ForceNewPage:= AppDetails.OutletStartNewPage;

//--Location Subtotal Bands
    QRGLocation.Enabled:= ((ckbGroupRemoteLocation.Checked) and (cmbRemoteLocations.ItemIndex = 0) and (radGroup.Checked) and (chkRemoteLocationBreakdown.Checked));
    QRBLocationFooter.Enabled:= ((ckbGroupRemoteLocation.Checked) and (cmbRemoteLocations.Itemindex = 0) and (radGroup.Checked) and (chkRemoteLocationBreakdown.Checked));
    QRGLocation.ForceNewPage:= AppDetails.LocationStartNewPage;

//--Summary Subtotal Bands
    if (NettExcludeSalesTax) then begin
      txtTotalsNett.Expression:= 'SUM(qrStaffSales.NETTSOLD - qrStaffSales.SALESTAX)';
    end
    else begin
      txtTotalsNett.Expression:= 'SUM(qrStaffSales.NETTSOLD)';
    end;

//--Which Main Band to use
//  the real one
    DetailBand.Enabled:= radDGTDetails.Checked;
//  or the invisible one
    ChildBand.Enabled:= (not radDGTDetails.Checked);
  end;
end;
{*****************************************************************************}
procedure TformQRStaffSales.bndTitleBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
...
{*****************************************************************************}
procedure TformQRStaffSales.QRGOutletBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  STOutletUnits:= 0;
  STOutletGross:= 0.00;
  STOutletDiscount:= 0.00;
  STOutletSalesTax:= 0.00;
  STOutletNett:= 0.00;

  with (WBCSV) do begin
    if (Active) then begin
      SendString(DM.qrStaffSales.FieldByName('outletname').AsString);
      WriteLine;
    end;
  end;
end;
{*****************************************************************************}
procedure TformQRStaffSales.QRGLocationBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  STLocationUnits:= 0;
  STLocationGross:= 0.00;
  STLocationDiscount:= 0.00;
  STLocationSalesTax:= 0.00;
  STLocationNett:= 0.00;

  with (WBCSV) do begin
    if (Active) then begin
      SendString(DM.qrStaffSales.FieldByName('remotelocationname').AsString);
      WriteLine;
    end;
  end;
end;
{*****************************************************************************}
procedure TformQRStaffSales.QRGStaffBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  STStaffUnits:= 0;
  STStaffGross:= 0.00;
  STStaffDiscount:= 0.00;
  STStaffSalesTax:= 0.00;
  STStaffNett:= 0.00;
end;
{*****************************************************************************}
procedure TformQRStaffSales.QRGForBBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  PrintBand:= False;    //Jon 24-09-2003
...
{*****************************************************************************}
procedure TformQRStaffSales.QRGGroupBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  PrintBand:= False;    //Jon 24-09-2003

  STGroupUnits:= 0;
  STGroupGross:= 0.00;
  STGroupDiscount:= 0.00;
  STGroupSalesTax:= 0.00;
  STGroupNett:= 0.00;
  STGroupCount:= 0;
end;
{*****************************************************************************}
procedure TformQRStaffSales.DetailBandBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
var
  LocalUnits: Currency;
  LocalGross: Currency;
  LocalDiscount: Currency;
  LocalSalesTax: Currency;
  LocalNett: Currency;

begin
  with dm.qrStaffSales do begin
    //if item is deleted and no sales for it, don't print it
    if ((FieldByName('grosssold').AsFloat = 0) and (not FieldByName('whendeleted').IsNull)) then begin
      PrintBand := False;
    end;

    //increment totals
    LocalUnits:= FieldByName('amountsold').AsCurrency;
    LocalGross:= FieldByName('grosssold').AsCurrency;
    LocalDiscount:= FieldByName('discount').AsCurrency;
    LocalSalesTax:= FieldByName('salestax').AsCurrency;
    LocalNett:= FieldByName('nettsold').AsCurrency;

    if (NettExcludeSalesTax) then begin
      LocalNett:= LocalNett - LocalSalesTax;
    end;
  end;
  STOutletUnits:= STOutletUnits + LocalUnits;
  STOutletGross:= STOutletGross + LocalGross;
  STOutletDiscount:= STOutletDiscount + LocalDiscount;
  STOutletSalesTax:= STOutletSalesTax + LocalSalesTax;
  STOutletNett:= STOutletNett + LocalNett;

  STLocationUnits:= STLocationUnits + LocalUnits;
  STLocationGross:= STLocationGross + LocalGross;
  STLocationDiscount:= STLocationDiscount + LocalDiscount;
  STLocationSalesTax:= STLocationSalesTax + LocalSalesTax;
  STLocationNett:= STLocationNett + LocalNett;

  STStaffUnits:= STStaffUnits + LocalUnits;
  STStaffGross:= STStaffGross + LocalGross;
  STStaffDiscount:= STStaffDiscount + LocalDiscount;
  STStaffSalesTax:= STStaffSalesTax + LocalSalesTax;
  STStaffNett:= STStaffNett + LocalNett;

  STSIGUnits:= STSIGUnits + LocalUnits;
  STSIGGross:= STSIGGross + LocalGross;
  STSIGDiscount:= STSIGDiscount + LocalDiscount;
  STSIGSalesTax:= STSIGSalesTax + LocalSalesTax;
  STSIGNett:= STSIGNett + LocalNett;

  STForBUnits:= STForBUnits + LocalUnits;
  STForBGross:= STForBGross + LocalGross;
  STForBDiscount:= STForBDiscount + LocalDiscount;
  STForBSalesTax:= STForBSalesTax + LocalSalesTax;
  STForBNett:= STForBNett + LocalNett;

  STGroupUnits:= STGroupUnits + LocalUnits;
  STGroupGross:= STGroupGross + LocalGross;
  STGroupDiscount:= STGroupDiscount + LocalDiscount;
  STGroupSalesTax:= STGroupSalesTax + LocalSalesTax;
  STGroupNett:= STGroupNett + LocalNett;
  Inc(STGroupCount);

  lblNett.Caption:= CurrencyToStr(LocalNett);

  if (Sender is TQRBand) then begin
    with WBCSV do begin
      if (Active) then begin
        with DM.qrStaffSales do begin
          ...
          SendCurrency(LocalUnits);
          SendCurrency(FieldByName('averageunitprice').AsCurrency);
          SendCurrency(LocalGross);
          SendCurrency(LocalDiscount);
          SendCurrency(LocalSalesTax);
          SendCurrency(LocalNett);
          WriteLine;
        end;
      end;
    end;
...
{*****************************************************************************}
procedure TformQRStaffSales.QRBGroupFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  if (not formReports.radDGTGroups.Checked) then begin
    if (STGroupCount <= 1) then begin
  ...
  lblGroupUnits.Caption:= FormatFloat('###,###,###,##0.###',STGroupUnits);
  if (STGroupUnits <> 0) then begin
    lblGroupPrice.Caption:= CurrencyToStr((STGroupGross / STGroupUnits));
  end
  else begin
    lblGroupPrice.Caption:= '0.00';
  end;
  lblGroupGross.Caption:= CurrencyToStr(STGroupGross);
  lblGroupDiscount.Caption:= CurrencyToStr(STGroupDiscount);
  lblGroupSalesTax.Caption:= CurrencyToStr(STGroupSalesTax);
  lblGroupNett.Caption:= CurrencyToStr(STGroupNett);

  with WBCSV do begin
    if (Active) then begin
      if (formReports.radDGTGroups.Checked) then begin
        SendString(DM.qrStaffSales.FieldByName('itemgroupabbrev').AsString);
      end
      else begin
        SendString(Format(sSubTotalsFor,[DM.qrStaffSales.FieldByName('itemgroupabbrev').AsString]));
      end;

      if (formReports.radDGTDetails.Checked) then begin
        SendEmpty(2);
      end;

      SendCurrency(STGroupUnits);
      if (STGroupUnits <> 0) then begin
        SendCurrency(STGroupGross / STGroupUnits);
      end
      else begin
        SendCurrency(0.00);
      end;
      SendCurrency(STGroupGross);
      SendCurrency(STGroupDiscount);
      SendCurrency(STGroupSalesTax);
      SendCurrency(STGroupNett);
      WriteLine;
    end;
  end;
end;
{*****************************************************************************}
procedure TformQRStaffSales.ShowGroupFooter(B: Boolean);
begin
  txtGroupFooter.Enabled:= B;

  lblGroupUnits.Enabled:= B;
  lblGroupPrice.Enabled:= B;
  lblGroupGross.Enabled:= B;
  lblGroupDiscount.Enabled:= B;
  lblGroupSalesTax.Enabled:= B;
  lblGroupNett.Enabled:= B;

  shpGroupUnits.Enabled:= B;
  shpGroupPrice.Enabled:= B;
  shpGroupGross.Enabled:= B;
  shpGroupDiscount.Enabled:= B;
  shpGroupSalesTax.Enabled:= B;
  shpGroupNett.Enabled:= B;

  if (B) then begin
    QRBGroupFooter.Height:= ((25 * CurrentHeight) div CreatedHeight);
  end
  else begin
    QRBGroupFooter.Height:= ((4 * CurrentHeight) div CreatedHeight);
  end;
end;
{*****************************************************************************}
procedure TformQRStaffSales.QRBForBFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  if (dm.qrStaffSales.FieldByName('hideforb').AsInteger = 1) then begin
    PrintBand:= False;
    Exit;
  end;

  lblForBUnits.Caption:= FormatFloat('###,###,###,##0.###',STForBUnits);
  if (STForBUnits <> 0) then begin
    lblForBPrice.Caption:= CurrencyToStr((STForBGross / STForBUnits));
  end
  else begin
    lblForBPrice.Caption:= '0.00';
  end;
  lblForBGross.Caption:= CurrencyToStr(STForBGross);
  lblForBDiscount.Caption:= CurrencyToStr(STForBDiscount);
  lblForBSalesTax.Caption:= CurrencyToStr(STForBSalesTax);
  lblForBNett.Caption:= CurrencyToStr(STForBNett);

  with WBCSV do begin
    if (Active) then begin
      if (DM.qrStaffSales.FieldByName('forb').AsString = 'F') then begin
        SendString(sFoodSubtotals);
      end
      else begin
        SendString(sBevSubTotals);
      end;

      if (formReports.radDGTDetails.Checked) then begin
        SendEmpty(2);
      end;
      SendCurrency(STForBUnits);
      if (STForbUnits <> 0) then begin
        SendCurrency((STForBGross / STForBUnits));
      end
      else begin
        SendCurrency(0.00);
      end;
      SendCurrency(STForBGross);
      SendCurrency(STForBDiscount);
      SendCurrency(STForBSalesTax);
      SendCurrency(STForBNett);
      WriteLine;
    end;
  end;
end;
{*****************************************************************************}
procedure TformQRStaffSales.QRBStaffFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  lblStaffUnits.Caption:= FormatFloat('###,###,###,##0.###',STStaffUnits);
  if (STStaffUnits <> 0) then begin
    lblStaffPrice.Caption:= CurrencyToStr((STStaffGross / STStaffUnits));
  end
  else begin
    lblStaffPrice.Caption:= '0.00';
  end;
  lblStaffGross.Caption:= CurrencyToStr(STStaffGross);
  lblStaffDiscount.Caption:= CurrencyToStr(STStaffDiscount);
  lblStaffSalesTax.Caption:= CurrencyToStr(STStaffSalesTax);
  lblStaffNett.Caption:= CurrencyToStr(STStaffNett);

  with WBCSV do begin
    if (Active) then begin
      if (formReports.radDGTTotals.Checked) then begin
        SendString(dm.qrStaffSales.FieldByName('STAFFNAME').AsString);
      end
      else begin
        SendString(Format(sSubTotalsFor,[dm.qrStaffSales.FieldByName('STAFFNAME').AsString]));
      end;
      ...
      SendCurrency(STStaffUnits);
      if (STStaffUnits <> 0) then begin
        SendCurrency((STStaffGross / STStaffUnits));
      end
      else begin
        SendCurrency(0.00);
      end;
      SendCurrency(STStaffGross);
      SendCurrency(STStaffDiscount);
      SendCurrency(STStaffSalesTax);
      SendCurrency(STStaffNett);
      WriteLine;
    end;
  end;
end;
{*****************************************************************************}
procedure TformQRStaffSales.QRBLocationFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  lblLocationUnits.Caption:= FormatFloat('###,###,###,##0.###',STLocationUnits);
  if (STLocationUnits <> 0) then begin
    lblLocationPrice.Caption:= CurrencyToStr((STLocationGross / STLocationUnits));
  end
  else begin
    lblLocationPrice.Caption:= '0.00';
  end;
  lblLocationGross.Caption:= CurrencyToStr(STLocationGross);
  lblLocationDiscount.Caption:= CurrencyToStr(STLocationDiscount);
  lblLocationSalesTax.Caption:= CurrencyToStr(STLocationSalesTax);
  lblLocationNett.Caption:= CurrencyToStr(STLocationNett);

  with (WBCSV) do begin
    if (Active) then begin
      SendString('TOTALS FOR ' + DM.qrStaffSales.FieldByName('remotelocationname').AsString);

      if (formReports.radDGTDetails.Checked) then begin
        SendEmpty(2);
      end;
      SendCurrency(STLocationUnits);

      if (STLocationUnits <> 0) then begin
        SendCurrency((STLocationGross / STLocationUnits));
      end
      else begin
        SendCurrency(0.00);
      end;
      SendCurrency(STLocationGross);
      SendCurrency(STLocationDiscount);
      SendCurrency(STLocationSalesTax);
      SendCurrency(STLocationNett);
      WriteLine;
    end;
  end;
end;
{*****************************************************************************}
procedure TformQRStaffSales.QRBOutletFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  lblOutletUnits.Caption:= FormatFloat('###,###,###,##0.###',STOutletUnits);
  if (STOutletUnits <> 0) then begin
    lblOutletPrice.Caption:= CurrencyToStr((STOutletGross / STOutletUnits));
  end
  else begin
    lblOutletPrice.Caption:= '0.00';
  end;
  lblOutletGross.Caption:= CurrencyToStr(STOutletGross);
  lblOutletDiscount.Caption:= CurrencyToStr(STOutletDiscount);
  lblOutletSalesTax.Caption:= CurrencyToStr(STOutletSalesTax);
  lblOutletNett.Caption:= CurrencyToStr(STOutletNett);

  with WBCSV do begin
    if (Active) then begin
      SendString(Format(sTotalsForCap,[DM.qrStaffSales.FieldByName('outletname').AsString]));

      if (formReports.radDGTDetails.Checked) then begin
        SendEmpty(2);
      end;
      SendCurrency(STOutletUnits);

      if (STOutletUnits <> 0) then begin
        SendCurrency((STOutletGross / STOutletUnits));
      end
      else begin
        SendCurrency(0.00);
      end;
      SendCurrency(STOutletGross);
      SendCurrency(STOutletDiscount);
      SendCurrency(STOutletSalesTax);
      SendCurrency(STOutletNett);
      WriteLine;
    end;
  end;
end;
{*****************************************************************************}
procedure TformQRStaffSales.QRBSummaryAfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
begin
  with WBCSV do begin
    if (Active) then begin
      SendString(sTotalsCap);
      if (formReports.radDGTDetails.Checked) then begin
        SendEmpty(2);
      end;
      SendCurrency(txtTotalsUnits.Value.dblResult);
      SendCurrency(txtTotalsPrice.Value.dblResult);
      SendCurrency(txtTotalsGross.Value.dblResult);
      SendCurrency(txtTotalsDiscount.Value.dblResult);
      SendCurrency(txtTotalsSalesTax.Value.dblResult);
      SendCurrency(txtTotalsNett.Value.dblResult);
      WriteLine;
...
{*****************************************************************************}
procedure TformQRStaffSales.QRGSuperItemGroupBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  STSIGUnits:= 0;
  STSIGGross:= 0.00;
  STSIGDiscount:= 0.00;
  ...
  if ((TotalsOnly) or (dm.qrStaffSales.FieldByName('superitemgroupid').IsNull)) then begin
    PrintBand:= False;
    Exit;
  end;
  with WBCSV do begin
    if (Active) then begin
      SendString(dm.qrStaffSales.FieldByName('superitemgroup').AsString);
      WriteLine;
    end;
  end;
end;
{*****************************************************************************}
procedure TformQRStaffSales.QRBSuperItemGroupFooterBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  if (dm.qrStaffSales.FieldByName('superitemgroupid').IsNull) then begin
    PrintBand:= False;
    Exit;
  end;

  lblSIGUnits.Caption:= FormatFloat('###,###,###,##0.###',STSIGUnits);
  if (STSIGUnits <> 0) then begin
    lblSIGPrice.Caption:= CurrencyToStr((STSIGGross / STSIGUnits));
  end
  else begin
    lblSIGPrice.Caption:= '0.00';
  end;
  lblSIGGross.Caption:= CurrencyToStr(STSIGGross);
  lblSIGDiscount.Caption:= CurrencyToStr(STSIGDiscount);
  lblSIGSalesTax.Caption:= CurrencyToStr(STSIGSalesTax);
  lblSIGNett.Caption:= CurrencyToStr(STSIGNett);

  with WBCSV do begin
    if (Active) then begin
      SendString(Format(sSubTotals,[DM.qrStaffSales.FieldByName('superitemgroup').AsString]));

      if (formReports.radDGTDetails.Checked) then begin
        SendEmpty(2);
      end;
      SendCurrency(STSIGUnits);
      if (STSIGUnits <> 0) then begin
        SendCurrency((STSIGGross / STSIGUnits));
      end
      else begin
        SendCurrency(0.00);
      end;
      SendCurrency(STSIGGross);
      SendCurrency(STSIGDiscount);
      SendCurrency(STSIGSalesTax);
      SendCurrency(STSIGNett);
      WriteLine;
    end;
  end;
end;
...
begin
  case (cmbReportType.ItemIndex) of
    ...
    33: ReportType := rptypStaffDetails;
    34: ReportType := rptypOpenTables;
    35: ReportType := rptypTableSummary;
    36: ReportType := rptypTransList;
    37: ReportType := rptypVoids;
