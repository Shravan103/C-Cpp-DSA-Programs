...
create or alter procedure getstaffsales_order (
    poutletid varchar(8192),
    premotelocationid varchar(8192),
    pforb char(1),
    pstaffid varchar(8192),
    pitemgroupid varchar(8192),
    fromtime timestamp,
    totime timestamp)
returns (
    itemid integer,
...
begin
  ...
  for select itemid, amountsold, grosssold, salestax, discount,
    averageunitprice, nettsold, outletid, remotelocationid, staffid, staffname
    from getitemstaffsales_order (:poutletid, :premotelocationid, :pforb, :pstaffid, :pitemgroupid, :fromtime, :totime)
    order by itemid
...
GRANT EXECUTE ON PROCEDURE GETITEMSTAFFSALES_ORDER TO PROCEDURE GETSTAFFSALES_ORDER;
GRANT SELECT ON ITEM TO PROCEDURE GETSTAFFSALES_ORDER;
GRANT SELECT ON ITEMGRP TO PROCEDURE GETSTAFFSALES_ORDER;
GRANT SELECT ON SUPERITEMGROUP TO PROCEDURE GETSTAFFSALES_ORDER;
GRANT SELECT ON OUTLET TO PROCEDURE GETSTAFFSALES_ORDER;
GRANT SELECT ON REMOTELOCATION TO PROCEDURE GETSTAFFSALES_ORDER;

/* Existing privileges on this procedure */

GRANT EXECUTE ON PROCEDURE GETSTAFFSALES_ORDER TO SYSDBA;
...
create or alter procedure getitemstaffsales_order (
    poutletid varchar(8192),
    premotelocationid varchar(8192),
    pforb char(1),
    pstaffid varchar(8192),
...
begin
  ...
  select id
    from FN_GETFIRSTID(:pitemgroupid, 0)
    into :firstitemgroupid;

  for select staffid, staffname, itemid, outletid, remotelocationid, invoicedepartmentid,
             amountsold, grosssold, salestax, discount
        from getallitemstaffsales_order(:poutletid, :premotelocationid, :pstaffid, :fromtime, :totime)
        into :staffid, :staffname, :itemid, :outletid, :remotelocationid, :invoicedepartmentid,
             :amountsold, :grosssold, :salestax, :discount
  do begin
    ...
    itemcleared = 1;
/*So We don't have to use the getitems stored procedure*/
    if ((pforb <> '') or (firstitemgroupid > 0)) then begin
      ...
      if (((pforb <> '') and (igforb <> pforb)) or
          ((firstitemgroupid > 0) and (:igitemgroupid not in (select id from FN_GETINTEGERLIST(:pitemgroupid))))) then begin
        itemcleared = 0;
      end
    end
    ...
    if ((itemcleared = 1) and
        ((repalldepartments = 1) or
         ((repalldepartments = 0) and
          (((repdepartmentid is null) and (invoicedepartmentid is null)) or
           ((repdepartmentid is not null) and (invoicedepartmentid = repdepartmentid)))))) then begin
...
GRANT SELECT ON SYS TO PROCEDURE GETITEMSTAFFSALES_ORDER;
GRANT EXECUTE ON PROCEDURE FN_GETFIRSTID TO PROCEDURE GETITEMSTAFFSALES_ORDER;
GRANT EXECUTE ON PROCEDURE GETALLITEMSTAFFSALES_ORDER TO PROCEDURE GETITEMSTAFFSALES_ORDER;
GRANT SELECT ON ITEM TO PROCEDURE GETITEMSTAFFSALES_ORDER;
GRANT SELECT ON ITEMGRP TO PROCEDURE GETITEMSTAFFSALES_ORDER;
GRANT EXECUTE ON PROCEDURE FN_GETINTEGERLIST TO PROCEDURE GETITEMSTAFFSALES_ORDER;

/* Existing privileges on this procedure */

GRANT EXECUTE ON PROCEDURE GETITEMSTAFFSALES_ORDER TO SYSDBA;



SET TERM ^ ;

create or alter procedure getallitemstaffsales_order (
    poutletid varchar(8192),
    premotelocationid varchar(8192),
    pstaffid varchar(8192),
    fromtime timestamp,
...
begin
  if ((:poutletid = '-2') and (:premotelocationid = '-2')) then begin
    for select s.staffname, wb.staffid, il.itemid, i.departmentid,
               sum(il.qty), sum(il.ilamount), sum(il.salestax), sum(il.discountamount)
        from invoice i 
        ...
        inner join wblogin wb on (o.whoordered = wb.loginid) 
        inner join staff s on (wb.staffid = s.staffid) 
         where ((i.wheninvoiced >= :fromtime) and (i.wheninvoiced < :totime) and
                ((:pstaffid is null) or (:pstaffid = '') or (:pstaffid = '-1') or (s.staffid in (select id from FN_GETINTEGERLIST(:pstaffid)))))
         group by s.staffname, wb.staffid, il.itemid, i.departmentid
         order by s.staffname, wb.staffid, il.itemid, i.departmentid
          into :staffname, :staffid, :itemid, :invoicedepartmentid,
               :amountsold, :grosssold, :salestax, :discount
    do begin
      suspend;
    end
    ...
    for select s.staffname, wb.staffid, il.itemid, i.remotelocationid, i.departmentid,
               sum(il.qty), sum(il.ilamount), sum(il.salestax), sum(il.discountamount)
        from invoice i 
        inner join invline il on (i.invoiceid = il.invoiceid)
        inner join ordln ol on il.orderlineid = ol.orderlineid
        ...
        inner join staff s on (wb.staffid = s.staffid) 
         where ((i.wheninvoiced >= :fromtime) and (i.wheninvoiced < :totime) and
                ((:premotelocationid is null) or (:premotelocationid = '') or (:premotelocationid = '-1') or
                 ((:premotelocationid = '0') and (i.remotelocationid is null)) or
                 (i.remotelocationid in (select id from FN_GETINTEGERLIST(:premotelocationid)))) and
                ((:pstaffid is null) or (:pstaffid = '') or (:pstaffid = '-1') or (s.staffid in (select id from FN_GETINTEGERLIST(:pstaffid)))))
         group by s.staffname, wb.staffid, il.itemid, i.remotelocationid, i.departmentid
         order by s.staffname, wb.staffid, il.itemid, i.remotelocationid, i.departmentid
          into :staffname, :staffid, :itemid, :remotelocationid, :invoicedepartmentid,
               :amountsold, :grosssold, :salestax, :discount
    do begin
  ...
  else if (:premotelocationid = '-2') then begin
    for select s.staffname, wb.staffid, il.itemid, i.outletid, i.departmentid,
               sum(il.qty), sum(il.ilamount), sum(il.salestax), sum(il.discountamount)
        from invoice i 
        inner join invline il on (i.invoiceid = il.invoiceid)
        inner join ordln ol on il.orderlineid = ol.orderlineid
        ...
        inner join staff s on (wb.staffid = s.staffid) 
         where ((i.wheninvoiced >= :fromtime) and (i.wheninvoiced < :totime) and
                ((:poutletid is null) or (:poutletid = '') or (:poutletid = '-1') or
                 (i.outletid in (select id from FN_GETINTEGERLIST(:poutletid)))) and
                ((:pstaffid is null) or (:pstaffid = '') or (:pstaffid = '-1') or (s.staffid in (select id from FN_GETINTEGERLIST(:pstaffid)))))
         group by s.staffname, wb.staffid, il.itemid, i.outletid, i.departmentid
         order by s.staffname, wb.staffid, il.itemid, i.outletid, i.departmentid
          into :staffname, :staffid, :itemid, :outletid, :invoicedepartmentid,
               :amountsold, :grosssold, :salestax, :discount
    do begin
      suspend;
    end
    ...
    for select s.staffname, wb.staffid, il.itemid, i.outletid, i.remotelocationid, i.departmentid,
               sum(il.qty), sum(il.ilamount), sum(il.salestax), sum(il.discountamount)
        from invoice i 
        inner join invline il on (i.invoiceid = il.invoiceid)
        inner join ordln ol on il.orderlineid = ol.orderlineid
        ...
        inner join staff s on (wb.staffid = s.staffid) 
         where ((i.wheninvoiced >= :fromtime) and (i.wheninvoiced < :totime) and
                ...
                ((:premotelocationid is null) or (:premotelocationid = '') or (:premotelocationid = '-1') or
                 ...
                 (i.remotelocationid in (select id from FN_GETINTEGERLIST(:premotelocationid)))) and
                ((:pstaffid is null) or (:pstaffid = '') or (:pstaffid = '-1') or (s.staffid in (select id from FN_GETINTEGERLIST(:pstaffid)))))
         group by s.staffname, wb.staffid, il.itemid, i.outletid, i.remotelocationid, i.departmentid
         order by s.staffname, wb.staffid, il.itemid, i.outletid, i.remotelocationid, i.departmentid
          into :staffname, :staffid, :itemid, :outletid, :remotelocationid, :invoicedepartmentid,
               :amountsold, :grosssold, :salestax, :discount
    do begin
      if (remotelocationid is null) then remotelocationid = 0;
      suspend;
...
GRANT SELECT ON INVOICE TO PROCEDURE GETALLITEMSTAFFSALES_ORDER;
GRANT SELECT ON INVLINE TO PROCEDURE GETALLITEMSTAFFSALES_ORDER;
GRANT SELECT ON ORDLN TO PROCEDURE GETALLITEMSTAFFSALES_ORDER;
GRANT SELECT ON ANORD TO PROCEDURE GETALLITEMSTAFFSALES_ORDER;
GRANT SELECT ON WBLOGIN TO PROCEDURE GETALLITEMSTAFFSALES_ORDER;
GRANT SELECT ON STAFF TO PROCEDURE GETALLITEMSTAFFSALES_ORDER;
GRANT EXECUTE ON PROCEDURE FN_GETINTEGERLIST TO PROCEDURE GETALLITEMSTAFFSALES_ORDER;

/* Existing privileges on this procedure */

GRANT EXECUTE ON PROCEDURE GETALLITEMSTAFFSALES_ORDER TO SYSDBA;
